<script lang="ts">
	...
	$: pendingRevealByLane = state.lanes.reduce<
		Record<number, { player: boolean; opponent: boolean }>
	>((acc, lane) => {
		acc[lane.index] = {
			player: state.pendingReveal.player.some(
				(entry) => entry.laneIndex === lane.index
			),
			opponent: state.pendingReveal.opponent.some(
				(entry) => entry.laneIndex === lane.index
			)
		};
		return acc;
...
<div class="board">
	...
	<div class="board__opponent-hand">
		Opponent hand: {state.players.opponent.hand.length} cards
	</div>
	...
	<div class="board__lanes">
		{#each state.lanes as lane (lane.index)}
			<Lane
				{lane}
				{selectedCardUid}
				showPlayerPendingReveal={pendingRevealByLane[lane.index]?.player ??
					false}
				showOpponentPendingReveal={pendingRevealByLane[lane.index]?.opponent ??
					false}
