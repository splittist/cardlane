<script lang="ts">
	import { describeKeyword, formatKeywordName } from '$lib/game/keywords';
	...
	$: keywordEntries = Object.entries(card.keywords ?? {}).filter(([, value]) =>
		Boolean(value)
	);
</script>
	...
	{#if keywordEntries.length > 0}
		<div class="card__keywords">
			{#each keywordEntries as [keyword, value] (keyword)}
				<span
					title={describeKeyword(keyword, value)}
					aria-label={describeKeyword(keyword, value)}
				>
					{formatKeywordName(keyword, value)}
				</span>
			{/each}
		</div>
	{/if}
