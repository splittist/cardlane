...
<button class:selectable={isSelectable} class={`lane ${terrainClass}`} type="button" on:click={() => dispatch('laneSelected', { laneIndex: lane.index })}>
  <div class="lane__label">Lane {lane.index + 1}</div>
  {#key `${lane.terrain ?? 'none'}:${lane.index}`}
    <div class="lane__terrain" in:scale={{ duration: 220, start: 0.92 }} out:fade={{ duration: 140 }}>
  ...
  <div class="lane__slot lane__slot--top">
    {#key `${lane.opponentCard?.uid ?? 'open'}:${lane.opponentCard?.currentHealth ?? 0}:${showOpponentPendingReveal ? 'queued' : 'clear'}`}
      <div class="lane__slot-content" in:scale={{ duration: 180, start: 0.92 }} out:fade={{ duration: 120 }}>
...
<style>
  .lane {
    ...
    color: #f1f4ff;
    text-align: center;
    transition: background 320ms ease, border-color 260ms ease, box-shadow 280ms ease;
  }
  ...
  .lane--flooded {
    border-color: #4a89de;
    background: linear-gradient(180deg, rgb(10 36 78 / 0.92), rgb(12 29 58 / 0.95));
    box-shadow: 0 0 0 1px rgb(74 137 222 / 0.3);
  }

  .lane--flooded::after {
    opacity: 1;
    background: radial-gradient(circle at 50% 15%, rgb(118 176 255 / 0.32), transparent 65%);
    animation: terrain-pulse 560ms ease-out;
  }

  .lane--overgrown {
    border-color: #4cb07a;
    background: linear-gradient(180deg, rgb(24 54 36 / 0.92), rgb(13 33 20 / 0.95));
    box-shadow: 0 0 0 1px rgb(76 176 122 / 0.3);
  }

  .lane--overgrown::after {
    opacity: 1;
    background: radial-gradient(circle at 50% 15%, rgb(137 236 164 / 0.24), transparent 66%);
    animation: terrain-pulse 560ms ease-out;
  }

  .lane--mud {
    border-color: #8b6b4a;
    background: linear-gradient(180deg, rgb(59 38 23 / 0.92), rgb(33 22 14 / 0.95));
    box-shadow: 0 0 0 1px rgb(139 107 74 / 0.3);
  }

  .lane--mud::after {
    opacity: 1;
    background: radial-gradient(circle at 50% 15%, rgb(196 156 110 / 0.22), transparent 66%);
    animation: terrain-pulse 560ms ease-out;
  }
