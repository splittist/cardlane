...
export function chooseAiAction(state: GameState, rng: () => number = Math.random): Action {
  const legalActions = getLegalActions(state, 'opponent');
  const playActions = legalActions.filter((action): action is Extract<Action, { type: 'play-card' }> => action.type === 'play-card');
