...
export function validateDeckDefinition(
	...
	for (const [cardId, copies] of copiesByCard.entries()) {
		if (copies > MAX_COPIES_PER_CARD) {
			throw new Error(
				`Deck cannot include more than ${MAX_COPIES_PER_CARD} copies of ${cardId}.`
			);
		}
	}
}
...
export function createDeck(
	owner: PlayerId,
	deckDefinition: readonly string[] = STARTING_DECK
): Card[] {
	validateDeckDefinition(deckDefinition);

	return deckDefinition.map((cardId, index) =>
		createCardInstance(cardId, owner, `${owner}-${cardId}-${index}`)
	);
}
